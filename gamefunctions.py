...
#This code creates four functions, purchase_item, new_random_monster, print_welcome and print_shop_menu
...
def purchase_item(itemPrice, startingMoney, quantityToPurchase):
    ...
    if money_leftover < 0:
        ...
        print(quantityToPurchase)
        return startingMoney
    else:
        print(quantityToPurchase)
        return money_leftover  # Enough money, return the remaining amount
leftovermoney = purchase_item(round((random.random()*10),3),round((random.random()*10),3),
                              round((random.randint(1, 10)),3))
leftovermoney2 = purchase_item(round((random.random()*10),3),round((random.random()*10),3),
                              round((random.randint(1, 10)),3))
leftovermoney3 = purchase_item(round((random.random()*10),3),round((random.random()*10),3),
                              round((random.randint(1, 10)),3))
print(leftovermoney)
...
def new_random_monster():
    """"outputs a random monster based on random inputs from a given set of values
